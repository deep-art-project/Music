...
def get_arguments():
	train_params = get_params('json_dir/train_params')
	wavenet_params=get_params('json_dir/wavenet_params')
	dataset_params = get_params('json_dir/dataset_params')
	return train_params,wavenet_params,dataset_params

def get_optimizer(model,optimizer_type,learning_rate,momentum):
	if optimizer_type =='sgd':
		return optim.sgd(model.praameters(),lr=learning_rate,momentum=momentum)
	if optimizer_type == 'RMSprop':
		return optim.RMSprop(model.praameters(),lr=learning_rate,momentum= momentum)
	if optimizer_type == 'Adam':
		return optim.Adam(model.praameters(),lr=learning_rate,momentum=momentum)	

def save_model(model,num_epoch,path):
	model_name = 'wavenet' + srt(num_epoch)+'.model'
...
def train():
	cuda_avaulable = torch.cuda.is_available()
	train_params,wavenet_params,dataset_params = get_arguments()
	net = wavenet_autoencoder(**wavenet_params)
	...
	if cuda_available is False :
		warnings.warn("Cuda is not avalable,",
	...
	for epoch in range(train_params['num_epochs']):
		for i_batch,sample_batch in enumerate(dataloader):
			...
			if cuda_avaulable:
				music_piece = music_piece.cuda()
				target_piece = target_piece.cuda()
			optimizer.zero_grad()
			...
			if num_trained%train_params['print_every'] ==0:
				...
				line = 'Average loss is %f' + str(avg_loss) +'\n'
				loss_log_file.writelines(line)
				loss_log_file.flush()
				total_loss =0

		if (epoch+1)%train_params['chech_point_every'] ==0:
			...
			save_models(net,epoch_trained+epoch+1,train_params['restore_dir'])
