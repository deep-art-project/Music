...
def download(data_dir):
    ...
    if os.path.exists(data_dir + 'downloaded.txt'):
        with open(data_dir + 'downloaded.txt', 'r') as f:
            downloaded_links = f.readlines()
    else:
        downloaded_links = []
        ...
        for composer in sources[genre]:
            ...
            for url in sources[genre][composer]:
                try:
                    ...
                    mid_re = r"mid$"
                    mid_links = bs_obj.find_all(
                        "a", {"href":re.compile(mid_re)}
                    )
                    links = mid_links
                    for link in links:
                        link = link.attrs['href']
                        if link[:4] != 'http':
                            link = parsed_url.hostname + '/' + link
                            link = "http://" + link
                        link = ''.join(c for c in link if c in valid_chars)
                        filename = link.split('/')[-1]
                        filepath = composer_dir + filename
                        print(link)
                        print(filepath)
                        whether_in = link not in downloaded_links
                        print(whether_in)
                        if link not in downloaded_links:
                            try:
                                response = urlopen(link)
                            except HTTPError:
                                print("Fail to access {}".format(link))
                            else:
                                with open(data_dir + 'downloaded.txt',
                                          'a') as f:
                                    f.writelines(link + '\n')
                                    f.flush()
                                f.close()
...
download("/home/zhaoyuekai/deep_art_proj/test_midi/")
